...
func (m *Manager) UpsertOpenDocument(ctx context.Context, in DocumentInput) error {
	...
	m.publishLocked([]DocumentKey{key}, !hadActive)
	return nil
}
...
func (m *Manager) CloseOpenDocument(ctx context.Context, uri string) error {
	...
	m.publishLocked([]DocumentKey{key}, hadActive != hasActive)
	return nil
}
...
func (m *Manager) RescanWorkspace(ctx context.Context) error {
	...
	m.publishLocked(changed, fullRebuild || m.snapshot.Load() == nil)
	return nil
}
...
func (m *Manager) publishLocked(changed []DocumentKey, fullRebuild bool) {
...
func (m *Manager) rescanLoop() {
	...
	for {
		...
		case <-m.rescanReset:
		case <-timer.C:
			_ = m.RescanWorkspace(context.Background())
		}
